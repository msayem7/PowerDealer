<template>
  <div class="projection-view">
    <header class="view-header">
      <h1 class="view-title">Cost Projection</h1>
      <p class="view-subtitle">Manage monthly gas cost projections for your customers</p>
    </header>
    
    <!-- Filters -->
    <div class="filters-bar">
      <div class="filter-group">
        <label for="customer-select">Customer (MPRN)</label>
        <select 
          id="customer-select"
      ...
      <div class="filter-actions" v-if="selectedMprn && selectedYear">
        ...
        >
          <svg v-if="saving" viewBox="0 0 24 24" fill="none" class="save-icon spinning">
            <path d="M12 4V2A10 10 0 0 0 2 12h2a8 8 0 0 1 8-8z" fill="currentColor"/>
          </svg>
          <svg v-else viewBox="0 0 24 24" fill="none" class="save-icon">
    ...
    <!-- Save Success -->
    <div v-if="showSaveSuccess" class="success-bar">
      <span>Projection saved successfully!</span>
    </div>

    <!-- Save Error -->
    <div v-if="saveError" class="error-bar">
      <span>{{ saveError }}</span>
      <button @click="saveData" class="retry-link">Retry</button>
    </div>
    ...
    <div class="view-content" v-else>
      ...
      <div v-else class="no-selection">
        ...
        <p>Select a customer and year to view cost projection</p>
      </div>
    </div>
  </div>
</template>
...
import ProjectionPivotGrid from '../components/projection/ProjectionPivotGrid.vue'
...
const saving = computed(() => projectionStore.saveLoading)
const saveError = computed(() => projectionStore.saveError)
const hasUnsavedChanges = computed(() => projectionStore.hasUnsavedChanges)
...
const refreshData = () => {
  console.log('[ProjectionView] Refresh data called')
  if (selectedMprn.value && selectedYear.value) {
    console.log('[ProjectionView] Calling fetchProjection from refreshData')
    projectionStore.fetchProjection(selectedMprn.value, selectedYear.value)
  }
}
...
// Watch for customer/year changes to fetch projection data
watch([selectedMprn, selectedYear], ([newMprn, newYear]) => {
  console.log('[ProjectionView] Watch triggered:', { newMprn, newYear })
  showSaveSuccess.value = false
  if (newMprn && newYear) {
    console.log('[ProjectionView] Calling fetchProjection from watch')
    projectionStore.fetchProjection(newMprn, newYear)
  }
})
...
.refresh-icon, .save-icon {
  width: 18px;
  height: 18px;
}

.refresh-icon.spinning, .save-icon.spinning {
